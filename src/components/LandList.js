...
export default function LandList(props) {
    const Item = ({ item }) => (
        <div style={{ flexDirection: 'row', display: 'flex', backgroundColor: 'white', marginBottom: 10, borderRadius: 10 }}>
            ...
            <div style={{ width: '80%' }}>
                <div style={{ color: 'black', fontWeight: 'bold' }}>{item.name}</div>
                <div style={{ color: 'black' }}>{item.city}</div>
            </div>
            <div style={{ width: '10%', display: 'flex', alignItems: 'center', justifyContent: 'center', }}>
                <button onClick={() => props.deleteLand(item.id)} className='btn btn-outline-danger' style={{ width: 30, height: 30, alignItems: 'center', justifyContent: 'center', display: 'flex' }} >
